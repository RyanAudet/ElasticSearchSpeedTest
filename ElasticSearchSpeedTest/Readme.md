...
```
    cd "ElasticSearchSpeedTest/test/ElasticSearchSpeedTest.Tests"
    dotnet test
```
...
```
    cd "ElasticSearchSpeedTest/src/ElasticSearchSpeedTest"
    dotnet lambda deploy-serverless
```

### Local Environment Setup

Must be deployed to AWS to create Elastic Search domain. Once domain is created local will need a reference to it in user secrets in order to execute locally.  
Add user secrets for AWS Region and Elastic Search URL.
...
Time to preform elastic search request is logged at critical level. Will appear in console (locally) and AWS Cloud watch (for deployed)
